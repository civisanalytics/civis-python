...
from civis.resources._resources import endpoint_class_docstring
...
import tenacity
from civis.response import ListResponse, PaginatedResponse, Response
...
"""
        ...
        for endpoint_name, endpoint_class in classes.items():
            ...
            method_defs = []
            class_docstring_added = False
            for method_name, method in vars(endpoint_class).items():
                method_def = ""
                if method_name.startswith("_"):
                    continue
                if not class_docstring_added:
                    class_docstring = endpoint_class_docstring(
                        endpoint_name, method_name
                    )
                    f.write(
                        f'    """{textwrap.indent(class_docstring, " " * 4).lstrip()}\n    """\n\n'  # noqa: E501
                    )
                    class_docstring_added = True
        ...
        f.write(
            f"""# Need the individual endpoint classes defined first as above,
# before we can define APIClient to use them.
class APIClient:
    \"""{textwrap.indent(civis.APIClient.__doc__, '    ').lstrip()}    \"""\n
    last_response: ListResponse | PaginatedResponse | Response | None
    def __init__(
        self,
        api_key: str | None = ...,
        return_type: str = ...,
    ...
    @property
    def default_credential(self) -> int | None: ...
    @property
    def default_database_credential_id(self) -> int | None: ...
    @property
