...
def list_models(job_type="train", author=SENTINEL, client=None, **kwargs):
    ...
    **kwargs : kwargs
    ...
    if job_type == "train":
        ...
        raise ValueError("Parameter 'job_type' must be None, 'train', " "or 'predict'.")
    ...
    models = client.scripts.list_custom(
        from_template_id=template_id_str, author=author, **kwargs
    )
    return models


def put_models_shares_users(
    id,
    user_ids,
    permission_level,
    client=None,
    ...
    return _share_model(
        id, user_ids, permission_level, entity_type="users", client=client, **kwargs
    )


def put_models_shares_groups(
    id,
    group_ids,
    permission_level,
    client=None,
    ...
    if share_email_body != "DEFAULT":
        kwargs["share_email_body"] = share_email_body
    return _share_model(
        id, group_ids, permission_level, entity_type="groups", client=client, **kwargs
    )


def _share_model(
    job_id, entity_ids, permission_level, entity_type, client=None, **kwargs
):
    """Share a container job and all run outputs with requested entities"""
    client = client or APIClient()
    if entity_type not in ["groups", "users"]:
        raise ValueError(
            "'entity_type' must be one of ['groups', 'users']. "
            "Got '{0}'.".format(entity_type)
        )

    log.debug(
        "Sharing object %d with %s %s at permission level %s.",
        job_id,
        entity_type,
        entity_ids,
        permission_level,
    ...
    for run in runs:
        ...
        for _output in outputs:
            if _output["object_type"] == "File":
                ...
                _func = getattr(client.json_values, "put_shares_" + entity_type)
                obj_permission = permission_level
            else:
                log.debug(
                    "Found a run output of type %s, ID %s; not sharing " "it.",
                    _output["object_type"],
                    _output["object_id"],
                )
                continue
...
def _unshare_model(job_id, entity_id, entity_type, client=None):
    ...
    log.debug(
        "Revoking permissions on object %d for %s %s.", job_id, entity_type, entity_id
    )
    ...
    for run in runs:
        ...
        for _output in outputs:
            if _output["object_type"] == "File":
                ...
                log.debug(
                    "Found run output of type %s, ID %s; not unsharing " "it.",
                    _output["object_type"],
                    _output["object_id"],
                )
                continue
