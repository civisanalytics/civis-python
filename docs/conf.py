...
# Docnames of pages that this file generates with exactly one linkable target apiece
# (an endpoint page has only its class docstring; a method page has only its one
# method). Any internal link that resolves to one of these docs can safely drop its
# "#anchor" suffix, since there's nothing else on the page to disambiguate -- see
# ``_strip_redundant_anchors``.
_SINGLE_TARGET_DOCNAMES = set()
...
def _write_method_rst(base_path, endpoint_name, method_name, civis_module):
    ...
    _SINGLE_TARGET_DOCNAMES.add(f"api_{base_path}/{method_name}")
    _write_if_changed(
        path,
        _METHOD_FMT.format(
...
def _write_resources_rst(class_names, filename, civis_module, class_data):
    ...
    for class_name in class_names:
        ...
        _SINGLE_TARGET_DOCNAMES.add(endpoint_rst_filename_no_ext)
...
def _strip_redundant_anchors(app, doctree, docname):
    """Drop "#anchor" from links that point at a ``_SINGLE_TARGET_DOCNAMES`` page.

    Sphinx always appends a target id to a cross-document link's URI, e.g. the
    "Methods" table on an endpoint page links to
    ``delete_containers_projects.html#civis.resources._resources.Scripts.delete_containers_projects``,
    and a method page's "A method of the X endpoint" links back to
    ``api_scripts_endpoint.html#api-scripts-endpoint``. Since each of those target pages
    has exactly one thing on it, the anchor is always redundant, so strip it here for a
    cleaner URL.
    """
    docdir = posixpath.dirname(docname)
    for node in doctree.findall(nodes.reference):
        refuri = node.get("refuri")
        if not refuri or "#" not in refuri or "://" in refuri:
            continue
        target_path, _, _ = refuri.partition("#")
        if not target_path:
            continue
        target_docname = posixpath.normpath(posixpath.join(docdir, target_path))
        if target_docname.endswith(".html"):
            target_docname = target_docname[: -len(".html")]
        if target_docname in _SINGLE_TARGET_DOCNAMES:
            node["refuri"] = target_path


def setup(app):
    app.add_css_file("custom.css")
    app.add_css_file("hide_endpoint_class_sig.css")
    app.connect("doctree-resolved", _strip_redundant_anchors)
