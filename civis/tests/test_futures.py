...
def _check_executor(from_template_id=None, max_workers=-1):
    job_id, run_id = 42, 43
...
@pytest.mark.parametrize('max_workers', [-1, 1, 5])
def test_container_scripts(max_workers):
    c = _check_executor(max_workers=max_workers)
    assert c.scripts.post_custom.call_count == 0
    assert c.scripts.post_containers.call_count > 0


@pytest.mark.parametrize('max_workers', [-1, 1, 5])
def test_custom_scripts(max_workers):
    c = _check_executor(from_template_id=133, max_workers=max_workers)
    assert c.scripts.post_custom.call_count > 0
    assert c.scripts.post_containers.call_count == 0
