...
TEST_TEMPLATE_ID_ALIAS_OBJECTS = [
    ...
    Response(
        dict(alias="civis-civisml-training-v2-3", object_id=TRAIN_ID_PROD)
    ),  # noqa
    Response(
        dict(alias="civis-civisml-prediction-v2-3", object_id=PREDICT_ID_PROD)
    ),  # noqa
    ...
    Response(
        dict(alias="civis-civisml-prediction-v1-4", object_id=PREDICT_ID_OLD)
    ),  # noqa
...
TEST_TEMPLATE_IDS = {  # Must match TEST_TEMPLATE_ID_ALIAS_OBJECTS
    None: {
        "training": TRAIN_ID_PROD,
        "prediction": PREDICT_ID_PROD,
        "registration": REGISTRATION_ID_PROD,
    },  # noqa
    "v2.3": {
        "training": TRAIN_ID_PROD,
        "prediction": PREDICT_ID_PROD,
    ...
    "v1.4": {
        "training": TRAIN_ID_OLD,
        "prediction": PREDICT_ID_OLD,
        "registration": None,
    },  # noqa
...
def test_stash_local_data_from_file(mock_file):
    ...
    mock_file.assert_called_once_with(
        mock.ANY, name="modelpipeline_data.csv", client=mock.ANY
    )
...
def test_modelfuture_constructor(mock_adc, mock_spe):
    ...
    mf = _model.ModelFuture(
        job_id=7, run_id=17, train_job_id=23, train_run_id=29, client=c
    )
...
@mock.patch.object(
    _model.cio, "file_id_from_run_output", mock.Mock(return_value=11, spec_set=True)
)
@mock.patch.object(
    _model.cio, "file_to_json", mock.Mock(return_value={"run": {"status": "succeeded"}})
)
@mock.patch.object(
    _model, "APIClient", return_value=create_client_mock_for_container_tests()
)
def test_modelfuture_pickle_smoke(mock_client):
    mf = _model.ModelFuture(
        job_id=7, run_id=13, client=create_client_mock_for_container_tests()
    )
    mf.result()
...
@mock.patch.object(
    ...
    "file_to_json",
    autospec=True,
    return_value={"run": {"status": "succeeded", "stack_trace": None}},
)
def test_set_job_exception_no_exception(mock_f2j):
    # If nothing went wrong, we shouldn't set an exception
    ro = [
        {"name": "model_info.json", "object_id": 137, "object_type": "File"},
...
@mock.patch.object(
    _model.cio, "file_to_json", mock.Mock(return_value="bar", spec_set=True)
)
@mock.patch.object(_model.ModelFuture, "_set_job_exception", lambda *args: None)
def test_getstate():
...
@mock.patch.object(
    _model.cio,
    "file_to_json",
    mock.Mock(spec_set=True, return_value={"run": {"status": "foo"}}),
)
def test_state():
    c = create_client_mock_for_container_tests(3, 7)
    ...
    c.scripts.get_containers_runs.return_value = Response(
        {"id": 7, "container_id": 3, "state": "failed"}
    )
    mf = _model.ModelFuture(3, 7, client=c)
...
@mock.patch.object(
    _model.ModelFuture,
    "metadata",
    return_value={"run": {"configuration": {"data": {"primary_key": "foo"}}}},
)
...
def test_table(mock_res, mock_lt, mock_meta):
    ...
    mf = _model.ModelFuture(3, 7, client=c)
    assert mf.table == "bar"
    mock_lt.assert_called_once_with(3, 7, "predictions.csv", index_col=0, client=c)


@mock.patch.object(
    _model.ModelFuture,
    "metadata",
    return_value={"run": {"configuration": {"data": {"primary_key": None}}}},
)
@mock.patch.object(_model, "_load_table_from_outputs", return_value="bar")
@mock.patch.object(_model.ModelFuture, "result")
@mock.patch.object(_model.ModelFuture, "_set_job_exception", mock.Mock())
def test_table_no_pkey(mock_res, mock_lt, mock_meta):
    ...
    mf = _model.ModelFuture(3, 7, client=c)
    assert mf.table == "bar"
    mock_lt.assert_called_once_with(3, 7, "predictions.csv", index_col=False, client=c)
...
def test_train_data_(mock_cio):
    ...
    path = "/green/eggs/and/ham"
    training_meta = {
        "run": {"configuration": {"data": {"location": path}}, "status": "succeeded"}
    }
    mock_cio.file_to_json.return_value = training_meta

    mf = _model.ModelFuture(
        job_id=1, run_id=2, train_job_id=11, train_run_id=13, client=mock_client
    )
...
def test_estimator(mock_le):
    ...
    assert mf.estimator == "spam"
    assert mock_le.call_count == 1, "The Estimator is only downloaded once and cached."


@mock.patch.object(_model.cio, "file_id_from_run_output", autospec=True)
@mock.patch.object(_model.cio, "file_to_json", return_value="foo", autospec=True)
@mock.patch.object(_model.ModelFuture, "_set_job_exception")
def test_validation_metadata_training(mock_spe, mock_f2f, mock_file_id_from_run_output):
    ...
    mock_file_id_from_run_output.assert_called_with(
        "metrics.json", 3, 7, client=mock.ANY
    )
...
@mock.patch.object(_model.ModelFuture, "_set_job_exception")
def test_validation_metadata_missing(mock_spe, mock_f2f, mock_file_id_from_run_output):
...
@mock.patch.object(_model.cio, "file_to_json", return_value="foo", autospec=True)
@mock.patch.object(_model.ModelFuture, "_set_job_exception")
def test_validation_metadata_prediction(
    mock_spe, mock_f2f, mock_file_id_from_run_output
):
    mock_file_id_from_run_output.return_value = 11
    c = create_client_mock_for_container_tests(3, 7)
...
@mock.patch.object(
    _model.cio,
    "file_to_json",
    mock.MagicMock(return_value={"metrics": "foo", "run": {"status": "succeeded"}}),
)
...
def test_metrics_training_None(mock_file_to_json, mock_file_id_from_run_output):
    mock_file_to_json.return_value = mock.MagicMock(
        return_value={"metrics": "foo", "run": {"status": "succeeded"}}
    )
    mock_file_id_from_run_output.return_value = 11
    c = create_client_mock_for_container_tests(3, 7)
    mf = _model.ModelFuture(3, 7, client=c)
...
@mock.patch.object(
    _model, "_get_template_ids_all_versions", mock.Mock(return_value=TEST_TEMPLATE_IDS)
)
@pytest.mark.parametrize(
...
def test__get_template_ids(version, train_id, predict_id, register_id):
    actual_train_id, actual_predict_id, actual_register_id = _model._get_template_ids(
        version, mock.ANY
    )
...
def test_modelpipeline_etl_init_err():
    ...
    with pytest.raises(NotImplementedError):
        _model.ModelPipeline(
            LogisticRegression(), "test", etl=LogisticRegression(), client=mock_client
        )
...
def test_modelpipeline_classmethod_constructor_errors(mock_future):
    ...
    response = namedtuple("Reponse", ["content", "response", "reason", "status_code"])(
        False, None, None, 404
    )
...
def test_modelpipeline_classmethod_constructor(mock_future):
    mock_client = mock.Mock()
    mock_client.scripts.get_containers.return_value = container = (
        _container_response_stub(TRAIN_ID_PROD)
    )
...
def test_modelpipeline_classmethod_constructor_nonint_id(m_sleep):
    ...
    mp = _model.ModelPipeline.from_existing(
        np.int64(1), np.int64(2), client=mock_client
    )

    out = json.dumps({"job": mp.train_result_.job_id, "run": mp.train_result_.run_id})
...
def test_modelpipeline_classmethod_constructor_old_version(
    ...
    mock_client = create_client_mock_for_container_tests()
    mock_client.scripts.get_containers.return_value = _container_response_stub(
        from_template_id=train_id
    )
    mp = _model.ModelPipeline.from_existing(1, 1, client=mock_client)
...
@pytest.mark.skipif(not HAS_PANDAS, reason="pandas not installed")
@mock.patch.object(_model, "_stash_local_dataframe", return_value=-11, autospec=True)
@mock.patch.object(_model.ModelPipeline, "_create_custom_run")
def test_modelpipeline_train_df(mock_ccr, mock_stash, mp_setup):
    ...
    mock_stash.assert_called_once_with(train_data, TRAIN_ID_PROD, client=mock.ANY)
    assert mp.train_result_ == "res"
...
def test_modelpipeline_state(mp_setup):
    ...
    with pytest.raises(ValueError, match="This model hasn't been trained yet."):
        mp.state
...
@pytest.mark.parametrize(
    ...
    [("v2.3", TRAIN_ID_PROD, PREDICT_ID_PROD), ("v1.4", TRAIN_ID_OLD, PREDICT_ID_OLD)],
)
def test_modelpipeline_pickling_preserves_template_ids(version, train_id, predict_id):
