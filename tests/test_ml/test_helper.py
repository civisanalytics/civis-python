...
def test_list_models():
    ...
    m_client.aliases.list.return_value = (
        test_model.TEST_TEMPLATE_ID_ALIAS_OBJECTS
    )  # noqa
...
def test_share_model_users():
    ...
    resp = helper._share_model(
        3, [7, 8], "write", "users", client=m_client, send_shared_email=True
    )
...
def test_share_model_groups():
    ...
    resp = helper._share_model(
        3, [7, 8], "write", "groups", client=m_client, send_shared_email=True
    )
...
def test_put_models_shares_groups(mock_share):
    ...
    mock_share.assert_called_once_with(
        1, [7, 8], "read", entity_type="groups", client=None
    )
...
def test_put_models_shares_users(mock_share):
    ...
    assert out == "retval"
    mock_share.assert_called_once_with(
        1, [7, 8], "read", entity_type="users", client=None
    )
